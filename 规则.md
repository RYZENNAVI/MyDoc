...
46. **标题应含关键构造名称（TITLE-1）**  
    - 大标题或小标题要明确指出本段讲解的具体 SQL 构造（大标题包含几乎所有的语句（概括一些），小标题则更加详细，包含所有当前语句如 `GROUP BY`、`HAVING`），方便快速检索；  
    - 如示例章节：`### 聚合函数 COUNT / SUM / AVG`。

47. **引用前置任务时使用 `[PARTREF::任务名称]`（REF-1）**  
    - 避免使用含糊表述“在前一个任务中”；  
    - 如果依赖的任务通过 `assumes` 引用，应明确给出 `[PARTREF::sql-basics]` 之类标记，便于读者跳转。

48. **统一使用 ASCII 双引号 " "（QUOTE-1）**  
    - 禁止使用全角或排版引号（“ ”）；  
    - LIKE、字符串常量示例中的引号同样遵循此规则。

49. **术语与示例中的标记风格一致（STYLE-1）**  
    - 技术关键字、列名、SQL 语句一律用反引号包裹；  
    - 示例中的引号风格保持统一，避免中英文引号混用；  
    - 建议对照示例提交 `977bc84` 检查一致性。

50. **提高 diff 可读性的编辑规范（DIFF-1）**  
    - 关闭编辑器软换行（soft wrapping），避免自动断行；（因为自动换行后的提交还是没有换行的！）  
    - 每行通常 ≤100，绝不超过 120 字符；  
    - 每个小标题后空一行；  
    - 将超链接放在独立行上，减少 diff 噪声。

51. **background的更加清晰的调整**  
    - 旧版本：Das Abfragen einer kleinen gesamten Tabelle kann gelegentlich ausreichen, um das gewünschte Ergebnis manuell zu überprüfen. Wenn jedoch das Ergebnis für eine neue Funktion benötigt wird und weiterverarbeitet werden soll, sind präzisere Abfrageangaben erforderlich, um entweder ein bestimmtes Ergebnis oder alle Ergebnisse zurückzugeben.
    - 新版本：Das Abfragen einer gesamten Tabelle kann gelegentlich ausreichen,
    ...
    结构清晰：提出条件（小表 → 可以手动查）→ 引出问题（大表或自动处理）→ 提出需求（需要更精细的方法）。

    引入了 „verfeinerte Mittel（精细的手段）” 作为总结性关键词，风格更抽象但更概括。

    🔸 语言评价：
    更符合现代教材风格，强调结构和可读性。

    更具抽象力（“automatische Weiternutzung” vs “weiterverarbeitet werden soll”）——强调数据的自动利用，而不是人工处理。

    用词更加规范，比如“abgelesen”更贴合数据库场景，而非“überprüfen”。
